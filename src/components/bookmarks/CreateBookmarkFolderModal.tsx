...
interface CreateBookmarkFolderModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function CreateBookmarkFolderModal({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await api.post("/bookmarks", { title: title.trim(), items: [] });
      setTitle("");
      onSuccess(res.data);
      onClose();
    } catch (err) {
      console.error("Erro ao criar pasta de bookmark:", err);
    } finally {
      ...
      <ModalContent className="p-6 rounded-3xl bg-card border border-border max-w-[500px] w-full">
        ...
        <ModalBody className="mt-4 gap-4">
          ...
          <Button
            ...
            className="rounded-xl mt-2 bg-foreground"
          >
